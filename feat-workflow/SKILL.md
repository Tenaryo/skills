...
1. **测试内容** - 说明当前测试测试的是什么功能点
2. **期望输出** - 说明该测试期望的输出是什么
3. **是否批准** - 询问架构师是否批准这个测试
...
如果测试文件和当前需要实现的内容强相关，即需要调用很多目前没有需要实现功能后才有的API，那么可以先跳到Phase 5中与架构师讨论设计，设计完后补充了那些依赖的API后再写测试，验证测试当前能编译但是失败的，然后再回去Phase 5实现具体功能。
